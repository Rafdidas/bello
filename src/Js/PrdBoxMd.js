...
const PrdBoxMd = ({prdData}) => {
    ...
    return(
        <div className="prd_box">
            ...
            {
                prdData.id <= 4 ? (
                    <img src={process.env.PUBLIC_URL + `../img/md_prd_img${prdData.id}.png`} alt="prd img"/>
                ) : (
                    <img src={
                        process.env.PUBLIC_URL + `../img/md_prd_img${Math.floor(Math.random() * 2) + 1}.png`
                    } alt="prd img" />
                )
            }
            </div>
